...
apartine (_, y) X = y == 0
apartine (x, _) Y = x == 0 
apartine (x, y) (DX dx m) = apartine (x - dx, y) m
apartine (x, y) (DY dy m) = apartine (x, y - dy) m
apartine (x, y) (U m1 m2) = apartine (x, y) m1 || apartine (x, y) m2 
...
rpf = undefined
--rfp [AConst x] = (Const x)
--rpf [AOp NEG] = Neg (Const x)
--rpf [AOp PLUS : e1 : e2] = rfp e1 :+: rpf e2
--rpf [AOp TIMES : e1 : e2] = rpf e1 :*: rpf e2
